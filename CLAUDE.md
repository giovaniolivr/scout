...
**Development and deploy are complete.** Scout is live in production (deployed via Railway). The portfolio site is also live and links to it. The project is now in the **document phase** — the academic submission document (`Scout.docx`, in the project root) needs to be updated to accurately reflect what was built.

**Document rules (strict):**
- Never write to `Scout.docx` or generate file content for it. All document work is discussed in chat only — the user writes the document themselves.
- When discussing diagrams or requirement tables, write the full content as plain text in the chat so the user can transcribe it.
- `Scout.docx` is not tracked in git (no need to push it).

**Document writing style (professor feedback):**
- Never use em-dashes (travessões: — or –) in any document text. Replace them with commas, parentheses, or restructure the sentence.
- Write in natural Brazilian Portuguese — avoid phrasing that sounds like it was generated by an AI. Avoid openers like "Diante desse cenário" and conclusions like "Os resultados demonstram que é possível". Prefer direct, student-written prose.
- Screen descriptions in section 4.2 must be brief and focused only on the purpose of the screen. Do not describe layout details — just state what the screen is for and what it enables the user to do.

**Two diagrams to update:**
- **Figura 1 — Diagrama de Usabilidade (Use Case):** Must reflect every action every actor (Candidato, Empresa, Sistema) can perform, plus all `<<include>>` and `<<extend>>` relationships between use cases. Full spec discussed in conversation on 2026-04-16.
- **Figura 2 — Diagrama de Classes:** Must reflect all Django models (User, CandidateProfile, CompanyProfile, Job, JobApplication, SkillEndorsement, CompanyFollow, CompanyQualityEndorsement) with attributes, methods, and multiplicities. Full spec discussed in conversation on 2026-04-16.

**Known discrepancies between the document and the actual system (to fix while writing):**
- RF001 text is blank in the document — needs to be filled in.
- Table 8 (Pontuação e Recomendações) states score range 0–1000; the actual external Scout Score is 0–100.
- Tables 7 and 8 use the word "entrevista" — Scout has no separate interview scheduling step; evaluation happens after the company sets the application status to Aprovado or Reprovado.
- Table 8 mentions "decaimento temporal" and "controles antifraude" — these are not implemented.
- The `internal_score` field was added on 2026-04-16 (migration 0011).
